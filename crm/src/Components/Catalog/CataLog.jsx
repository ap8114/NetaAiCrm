...
import { createProducts, fetchAllProducts, updateProducts } from "../../slices/productSlice";
import { createServices, fetchAllServices, updateServices } from "../../slices/serviecSlice";
...
const CatalogTabs = () => {
  ...
  useEffect(() => {
    const loadCatalog = async () => {
      try {
        ...
        // Normalize and merge products and services with type field
        // Expecting productsResult and servicesResult arrays
        const products = productsResult.map(p => ({
          ...p,
          type: "Product",
          price: p.unit_price !== undefined ? p.unit_price : p.price,
          cost: p.unit_cost !== undefined ? p.unit_cost : undefined,
          unitPrice: p.unit_price,
          unitCost: p.unit_cost,
          extendedDescription: p.extended_description,
          unitOfMeasure: p.unit_of_measure,
          isSubitem: p.is_subitem,
        }));
        const services = servicesResult.map(s => ({
          ...
          price: s.price,
          cost: undefined,
          defaultScope: s.default_scope_of_work,
          isSubitem: s.is_subitem,
        }));
        setCatalogItems([...products, ...services]);
      } catch (error) {
  ...
  const handleMaterialChange = (index, field, value) => {
    const updated = [...materials];
    updated[index][field] =
      field === "quantity" || field === "cost" ? parseFloat(value) || 0 : value;
    setMaterials(updated);
  };
  ...
  const totalCost = materials.reduce(
    (acc, mat) => acc + mat.quantity * mat.cost,
    0
  );

  // Open modal and populate form for edit or clear for new
  const handleShow = (item = null, idx = null) => {
    if (item) {
      setEditIndex(idx);
      setItemType(item.type || "Product");
      setFormData({
        ...
        description: item.description || "",
        extendedDescription: item.extendedDescription || "",
        tags: item.tags ? item.tags.join ? item.tags.join(", ") : item.tags : "",
        unitOfMeasure: item.unitOfMeasure || "",
        unitPrice:
  ...
  // Save handler with conditional redux dispatch
    ...
    try {
      if (itemType === "Product") {
        const productPayload = {
          ...
          extended_description: formData.extendedDescription,
          tags: tagsArray,
        ...
        if (editIndex !== null) {
          await dispatch(updateProducts({ id: catalogItems[editIndex].id, data: productPayload })).unwrap();
        } else {
        ...
        const servicePayload = {
          name: formData.name,
          description: formData.description,
          default_scope_of_work: formData.defaultScope,
          tags: tagsArray,
          price: Number(formData.price) || 0,
          is_subitem: formData.isSubitem,
          is_template: true,
        };
        if (editIndex !== null) {
          await dispatch(updateServices({ id: catalogItems[editIndex].id, data: servicePayload })).unwrap();
        } else {
          await dispatch(createServices(servicePayload)).unwrap();
        }
      }
  ...
  return (
    <Container fluid className="p-4">
                    ...
                    <td>
                      <InputGroup>
                        <Form.Control
                          value={mat.item}
                          onChange={(e) =>
                            handleMaterialChange(idx, "item", e.target.value)
                          }
                        />
                        <Button
      ...
      <Modal show={showModal} onHide={handleClose} centered>
        <Modal.Header closeButton>
          <Modal.Title>{editIndex !== null ? "Edit" : "Add"} Catalog Item</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            <Row className="mb-3">
