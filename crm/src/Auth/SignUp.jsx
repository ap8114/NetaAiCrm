...
const SignUp = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const { name, email, password, refferl, phone_number } = formData;
    dispatch(registerUser({ name, email, password, refferl, phone_number }));
  };

  return (
    <div className="min-vh-100 d-flex justify-content-center align-items-center" style={{ background: "#fff" }}>
      <div className="rounded-4 shadow-lg bg-white w-100" style={{ maxWidth: 700 }}>
        <div className="row g-0 h-100">
          <div className="col-md-6 d-flex flex-column justify-content-center p-3">
            <div className="logo-container mb-3 d-flex justify-content-center">
              <Link to="/"><img src={bonbonlogo} alt="Logo" className="img-fluid" style={{ maxWidth: "110px" }} /></Link>
            </div>
            <h5 className="text-center mb-3 fw-bold" style={{ color: "#333" }}>Create Account</h5>
            {error && <div className="alert alert-danger py-1">{error}</div>}
            <form onSubmit={handleSubmit}>
              {/* same fields as before */}
              {/* ... */}
              <button type="submit" className="btn ai-premium-btn w-100 py-1" disabled={loading}>
                {loading ? "Signing Up..." : "Sign Up"}
              </button>
            </form>
            <p className="text-center mt-2 small">
              Already have an account?
              <Link to="/"><button className="btn btn-link p-0 ms-1 text-decoration-none">Login</button></Link>
            </p>
          </div>
          <div className="col-md-6 d-none d-md-block p-0">
            <img src={bonbo2} alt="Sign Up Visual" className="img-fluid w-100 h-100" style={{ objectFit: "cover", borderTopRightRadius: "1.5rem", borderBottomRightRadius: "1.5rem" }} />
          </div>
        </div>
      </div>
    </div>
